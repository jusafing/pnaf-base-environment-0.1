...
TCPDUMP_DIR="/paf/dem/pasm/tcpdump"
TCPDUMP_RULESDIR='/usr/local/tcpdump'
TCPDUMP_WEB="http://www.rtfm.com/tcpdump/tcpdump-0.9b3.tar.gz"
TCPDUMP_SRCFILE="$INSTALLER_WORKDIR/build/tcpdump-$TCPDUMP_VERSION.tar.gz"
...
{
    local fname="start"
    logMsg "$fname" "Installing Tcpdump IDS version $TCPDUMP_VERSION"
    userInstall
    checkArgs
    installTcpdump
}
...
installTcpdump()
{
    local fname="installTcpdump"
    logMsg "$fname" "Installing Tcpdump v$TCPDUMP_VERSION"
    logMsg "$fname" "Using Tcpdump source file $TCPDUMP_SRCFILE"
    ...
    execCmd "ldconfig"
    execCmd "cd $INSTALLER_WORKDIR/build"
    cleanDir "tcpdump-$TCPDUMP_VERSION"
    execCmd "tar -zxvf tcpdump-$TCPDUMP_VERSION.tar.gz"
    execCmd "cd tcpdump-$TCPDUMP_VERSION"
    execCmd "./configure --prefix=$TCPDUMP_DIR"
    execCmd "make"
    execCmd "make install"
}
##############################################################################
