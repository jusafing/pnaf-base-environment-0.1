...
INSTALLER_ID="7/23 CXTRACKER"
INSTALLER_USER=`whoami`
INSTALLER_NARG=$#
INSTALLER_ARG1=$1
INSTALLER_ARG2=$2
#----------------------------------------------------------------------------#
CXTRACKER_DIR="/pnaf/dpm/nfae/cxtracker"
CXTRACKER_WEB="https://github.com/gamelinux/cxtracker.git"
CXTRACKER_SRCFILE="$INSTALLER_WORKDIR/build/cxtracker.tar.gz"

##############################################################################
start()
{
    local fname="start"
    logMsg "$fname" "Installing Cxtracker"
    userInstall
    checkArgs
    installCxtracker
}
...
installAptPackages()
{
    ...
    execCmd "aptitude install -y libpcap-dev libpcre3-dev libnet-pcap-perl \
	     libgetopt-long-descriptive-perl git-core libdatetime-perl" 
}
##############################################################################
installCxtracker()
{
    local fname="installCxtracker"
    logMsg "$fname" "Installing Cxtracker"
    logMsg "$fname" "Using Cxtracker source file $CXTRACKER_SRCFILE"
    cleanDir "$CXTRACKER_DIR"
    execCmd "mkdir -p $CXTRACKER_DIR"
    execCmd "cd $INSTALLER_WORKDIR/build"
    cleanDir "cxtracker"
    execCmd "tar -zxvf cxtracker.tar.gz"
    execCmd "cd cxtracker/src"
    execCmd "make"
    execCmd "cp cxtracker ../bin"
    execCmd "cp -r ../../cxtracker/* $CXTRACKER_DIR"
    execCmd "chmod 755 $CXTRACKER_DIR/bin/*"
}
##############################################################################
start
